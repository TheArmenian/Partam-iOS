...
- (void)touchDown:(id)sender {
    for (UIButton * btn in headers) {
        if ([sender tag] == btn.tag && ([sender tag] == 0 ||[sender tag] == 1)) {
    ...
    if (allowsMultipleSelection) {
        ...
        if ([selectionIndexes containsIndex:1]) {
            if (self.allowsEmptySelection || [selectionIndexes count] > 1) {
...
- (void)layoutSubviews {
    if (self.isHorizontal) {
        ...
        [self performSelector:@selector(animationStopped) withObject:nil afterDelay:0.7];
    }
}
...
//        return;
//    }
//     NSMutableIndexSet *miss = [selectionIndexes mutableCopy];
//    for (UIButton * btn in headers) {
//        if (btnTag == btn.tag) {
//            btn.selected = !btn.selected;
//        } else {
...
//    if (btnTag == 0) {
//        [miss removeIndex:1];
//        UIButton * btn= headers[1];
//        btn.selected = NO;
//    } else if(btnTag == 1) {
//        [miss removeIndex:0];
//        UIButton * btn= headers[0];
//        btn.selected = NO;
//    }
//    
//    btnTag = -1;
//    [self setSelectionIndexes:miss];
