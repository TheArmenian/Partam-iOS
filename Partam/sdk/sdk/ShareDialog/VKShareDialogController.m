    ...
    if (self.ownerId.integerValue > 0) {
        [[[VKApi users] get:@{VK_API_USER_ID : self.ownerId, VK_API_FIELDS : @"first_name_acc,can_post,sex,exports"}] executeWithResultBlock:^(VKResponse *response) {
            ...
            if (user.id.integerValue == currentUserId) {
                self.hintLabel.text = [NSString stringWithFormat:@"NewPostHint%@",@"YourWall"];
                ...
                NSString * userHintText = [NSString stringWithFormat:@"UserWall%@", user.first_name_acc];
                self.hintLabel.text = [NSString stringWithFormat:@"NewPostHint%@", userHintText];
        ...
        self.hintLabel.text = [NSString stringWithFormat:@"NewPostHint%@", @"GroupWall"];
...
-(NSArray*) rightBarButtonItems {
    if (!_sendButton) {
        _sendButton = [[UIBarButtonItem alloc] initWithTitle:@"Done" style:UIBarButtonItemStyleDone target:self action:@selector(sendMessage:)];
    }
    return @[_sendButton];
...
-(void)denyPostingOnWall:(NSString*)ownerName {
    ...
    self.textView.editable  = NO;
    self.textView.textColor = [UIColor lightGrayColor];
    self.textView.text      = [NSString stringWithFormat:@"DenyPostingOn%@Wall", ownerName];
}
...
//    if (exports.count) [post addExtraParameters:@{VK_API_SERVICES : [exports componentsJoinedByString:@","]}];
    ...
    [post executeWithResultBlock:^(VKResponse *response) {
        ...
        [self.textView becomeFirstResponder];
        [[[UIAlertView alloc] initWithTitle:nil message:@"ErrorWhilePosting" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
    }];
}
...
-(void) createAttachments {
    ...
    self.textView.textAlignment = NSTextAlignmentLeft;
    self.textView.placeholder = @"NewMessageText";
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (attach.targetUpload) {
        ...
        cell.attachLabel.text = [NSString stringWithFormat:@"Link%@", attach.attachmentString];
        [cell setOnDeleteBlock:^{
            [self removeAttachIfExists:attach];
        }];
        return cell;
